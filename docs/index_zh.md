# 文档索引

## 核心文档

- 项目总览（英文）：[README](../README.md)
- 项目总览（中文）：[docs/README_zh.md](README_zh.md)
- 架构说明（英文）：[ARCHITECTURE](../ARCHITECTURE.md)
- 架构说明（中文）：[docs/ARCHITECTURE_zh.md](ARCHITECTURE_zh.md)
- Runtime Guide（英文）：[docs/python-runtime-guide.md](python-runtime-guide.md)
- Runtime Guide（中文）：[docs/python-runtime-guide_zh.md](python-runtime-guide_zh.md)
- Navigation（英文）：[docs/NAVIGATION.md](NAVIGATION.md)
- Navigation（中文）：[docs/NAVIGATION_zh.md](NAVIGATION_zh.md)

## Navigation 内容层

- 系统地图：
  - 英文：[docs/navigation-docs/system-map.md](navigation-docs/system-map.md)
  - 中文：[docs/navigation-docs/system-map_zh.md](navigation-docs/system-map_zh.md)
- 模块索引：
  - 英文：[docs/navigation-docs/module-index.md](navigation-docs/module-index.md)
  - 中文：[docs/navigation-docs/module-index_zh.md](navigation-docs/module-index_zh.md)
- 工作流待办：
  - 英文：[docs/navigation-docs/workstream-backlog.md](navigation-docs/workstream-backlog.md)
  - 中文：[docs/navigation-docs/workstream-backlog_zh.md](navigation-docs/workstream-backlog_zh.md)

## 设计文档

- Python 运行时重构设计：
  - 英文：[docs/design-docs/python-runtime-refactor.md](design-docs/python-runtime-refactor.md)
  - 中文：[docs/design-docs/python-runtime-refactor_zh.md](design-docs/python-runtime-refactor_zh.md)

## 产品规格

- Python 运行时行为规格：
  - 英文：[docs/product-specs/python-runtime-v2.md](product-specs/python-runtime-v2.md)
  - 中文：[docs/product-specs/python-runtime-v2_zh.md](product-specs/python-runtime-v2_zh.md)
- A 股股票池契约：
  - 英文：[docs/product-specs/a-share-universe-contract.md](product-specs/a-share-universe-contract.md)
  - 中文：[docs/product-specs/a-share-universe-contract_zh.md](product-specs/a-share-universe-contract_zh.md)

## 执行计划

- 进行中计划：`docs/exec-plans/active/`
- 已完成计划：`docs/exec-plans/done/`

## 任务日志

- 结构化日志（中英文）：`docs/logs/`
