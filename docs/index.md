# Documentation Index

## Core Docs

- Project overview (EN): [README](../README.md)
- Project overview (ZH): [docs/README_zh.md](README_zh.md)
- Architecture (EN): [ARCHITECTURE](../ARCHITECTURE.md)
- Architecture (ZH): [docs/ARCHITECTURE_zh.md](ARCHITECTURE_zh.md)
- Runtime guide (EN): [docs/python-runtime-guide.md](python-runtime-guide.md)
- Runtime guide (ZH): [docs/python-runtime-guide_zh.md](python-runtime-guide_zh.md)
- Navigation (EN): [docs/NAVIGATION.md](NAVIGATION.md)
- Navigation (ZH): [docs/NAVIGATION_zh.md](NAVIGATION_zh.md)

## Navigation Content

- System map:
  - EN: [docs/navigation-docs/system-map.md](navigation-docs/system-map.md)
  - ZH: [docs/navigation-docs/system-map_zh.md](navigation-docs/system-map_zh.md)
- Module index:
  - EN: [docs/navigation-docs/module-index.md](navigation-docs/module-index.md)
  - ZH: [docs/navigation-docs/module-index_zh.md](navigation-docs/module-index_zh.md)
- Workstream backlog:
  - EN: [docs/navigation-docs/workstream-backlog.md](navigation-docs/workstream-backlog.md)
  - ZH: [docs/navigation-docs/workstream-backlog_zh.md](navigation-docs/workstream-backlog_zh.md)

## Design Docs

- Python runtime refactor design:
  - EN: [docs/design-docs/python-runtime-refactor.md](design-docs/python-runtime-refactor.md)
  - ZH: [docs/design-docs/python-runtime-refactor_zh.md](design-docs/python-runtime-refactor_zh.md)

## Product Specs

- Python runtime behavior specification:
  - EN: [docs/product-specs/python-runtime-v2.md](product-specs/python-runtime-v2.md)
  - ZH: [docs/product-specs/python-runtime-v2_zh.md](product-specs/python-runtime-v2_zh.md)
- A-share universe contract:
  - EN: [docs/product-specs/a-share-universe-contract.md](product-specs/a-share-universe-contract.md)
  - ZH: [docs/product-specs/a-share-universe-contract_zh.md](product-specs/a-share-universe-contract_zh.md)

## Execution Plans

- Active plans: `docs/exec-plans/active/`
- Completed plans: `docs/exec-plans/done/`

## Logs

- Structured task logs (EN + ZH): `docs/logs/`
