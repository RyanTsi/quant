...
- 模型域股票池契约现在统一位于 `model_function/universe.py`，共享的 Qlib 训练/评估/可视化 helper 位于 `model_function/qlib.py`；不要在 adapter 或脚本里重复实现这些逻辑。
