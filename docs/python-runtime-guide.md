...
- Runtime path: `runtime.tasks.train_model()` -> `ModelPipelineService.train_model()` -> `alpha_models.qlib_workflow.main()`
- Important behavior:
  - the Qlib workflow records `qlib_train` metadata used by later predict/eval/view commands
...
- Canonical logic: `runtime/adapters/modeling.generate_predictions`
- Important behavior:
...
- Canonical logic: `runtime/adapters/modeling.build_portfolio_outputs`
- Important behavior:
  - input is `output/top_picks_<date>.csv`
  - outputs are `output/target_weights_<date>.csv` and `output/orders_<date>.csv`
  - `build_portfolio` run history records the picks file, output files, and order summary stats
...
- Predict fails with `No trained model found`:
  - set `QLIB_RECORDER_ID` and `QLIB_EXPERIMENT_ID`, or run `train` first so `qlib_train` history is recorded.
